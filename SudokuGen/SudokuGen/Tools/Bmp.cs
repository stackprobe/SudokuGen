...
namespace Charlotte.Tools
{
	public class Bmp
	{
		...
		public void putOptionBmp(Bitmap optBmp)
		{
			int l = (table.w - optBmp.Width) / 2;
			int t = table.h + 5; // + margin

			for (int x = 0; x < optBmp.Width; x++)
			{
				for (int y = 0; y < optBmp.Height; y++)
				{
					table.set(l + x, t + y, Dot.getDot(optBmp.GetPixel(x, y)));
				}
			}
		}
	}
